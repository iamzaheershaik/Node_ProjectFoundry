...
exports.viewAllSubcategories = async (req, res) => {
  try {
    ...
    // Fetch extracategory counts for each subcategory
    let subcategoriesWithCount = await Promise.all(
      subcategories.map(async (sub) => {
        let extracategoryCount = await Extracategory.countDocuments({
          subcategory: sub._id,
        });
        return { ...sub.toObject(), extracategoryCount };
      }),
    );
...
exports.viewSingleSubcategory = async (req, res) => {
  try {
    ...
    // Fetch extracategories that belong to this subcategory
    let extracategories = await Extracategory.find({
      subcategory: req.params.id,
    });

    return res.render("subcategory/singleView", {
      subcategory,
      user,
      extracategories,
    });
    ...
    if (req.file) {
      ...
      if (oldSubcategory.subcategoryImage) {
        let oldPath = path.join(
          __dirname,
          "..",
          oldSubcategory.subcategoryImage,
        );
...
exports.deleteSubcategory = async (req, res) => {
  try {
    ...
    // Cascade delete: remove all extracategories belonging to this subcategory (and their images)
    let extracategories = await Extracategory.find({
      subcategory: req.params.id,
    });
    ...
    console.log("Subcategory Deleted Success (with extracategories)");
    return res.redirect("/subcategory/view");
  } catch (error) {
    console.log(error);
    return res.redirect("/");
