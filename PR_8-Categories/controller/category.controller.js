    ...
    try {
        ...
        let categoriesWithCount = await Promise.all(
            categories.map(async (cat) => {
                let subcategoryCount = await Subcategory.countDocuments({ category: cat._id });
                return { ...cat.toObject(), subcategoryCount };
            })
        );

        return res.render("category/viewCategory", { categories: categoriesWithCount, user, search });
    } catch (error) {
        console.log(error);
        return res.redirect("/");
    }
...
exports.deleteCategory = async (req, res) => {
    try {
        ...
        console.log('Category Deleted Success (with subcategories)');
        return res.redirect("/category/view");
    } catch (error) {
        console.log(error);
        return res.redirect("/");
